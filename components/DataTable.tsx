...
export function DataTable<T>({
  ...
  return (
    <div className="space-y-3">
      {openFilterColumn ? (
        <div className="rounded-[16px] border border-line bg-[rgba(8,15,28,0.98)] p-4 shadow-[0_16px_36px_rgba(0,0,0,0.28)]" data-table-filter-root="true">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <div className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--accent)]">Вибір значень</div>
              <div className="mt-1 text-sm text-white">{openFilterLabel}</div>
            </div>
            <div className="flex flex-wrap gap-2">
              <button
                className="rounded-[10px] border border-line bg-[rgba(10,18,33,0.72)] px-3 py-2 text-xs font-semibold text-[var(--muted)] transition hover:text-white"
                    ...
                    <th className="relative whitespace-nowrap border-b border-line px-4 py-3 text-left font-semibold text-[var(--ink)]" key={header.id}>
                      {header.isPlaceholder ? null : (
                        <div className="flex items-center justify-between gap-3">
                          ...
                          {hasFilter ? (
                            <div data-table-filter-root="true">
                              <button
